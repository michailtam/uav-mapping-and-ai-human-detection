...
def launch_setup(context, *args, **kwargs):
    ...
    joy_config_file_path = os.path.join(
        get_package_share_directory('uav_mapping'), 'config','joy_config.yaml')
    
    nav2_params_file = os.path.join(
        get_package_share_directory('uav_navigation'), 'config', 'nav2_params.yaml')
    
    # rviz_config_file = os.path.join(
    #     get_package_share_directory('uav_mapping'), 'config', 'config.rviz')
    ...
    return [
        ...
        Node(
            ...
            parameters=[{
              'use_sim_time': use_sim_time,
              'fixed_frame_id':'base_link_stabilized', #TODO: this is not right link
